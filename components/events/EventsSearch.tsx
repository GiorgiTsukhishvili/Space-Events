...
import {
  EventsDetail,
  EventsSearchProps,
} from "../interfaces/eventsSearchInterface";

const EventsSearch = ({ onSearch }: EventsSearchProps) => {
  const [eventDetails, setEventDetails] = useState<EventsDetail>({
    year: "2021",
    month: "1",
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSearch(eventDetails.year, eventDetails.month);
  };

  const changeDetails = (value: string, key: string) => {
    setEventDetails((prevDetails) => {
      return { ...prevDetails, [value]: key };
    });
  };
  ...
  return (
    <form
      ...
      className="my-[2rem] sm:flex-row mx-auto shadow-lg p-[1rem] bg-white rounded-[8px] w-[90%] max-w-[40rem] flex justify-between flex-col gap-[1rem]"
    >
      <div className="w-full flex gap-4 flex-col sm:w-[80%] sm:flex-row">
        <div className="flex-1 flex gap-4 items-center justify-between">
          ...
          <select
            ...
            onChange={(e) => changeDetails("year", e.target.value)}
          >
        ...
        <div className="flex-1 flex gap-4 items-center justify-between">
          ...
          <select
            ...
            onChange={(e) => changeDetails("month", e.target.value)}
          >
